...
def add_type_works():
    with open("type_works/tipo_de_obras.csv", "r") as f_in:
        ...
        for line in lines[1:]:
            ...
            response = send(
                BASE_URL + "typeworks/add",
                {"flag": type_work[0], "name": type_work[1], "status_list": []},
            )
            if response.status_code == 200:
                uploaded = uploaded + 1
...
def add_type_works_work_statuses():
    with open("type_works/tipo_de_obras.csv", "r") as f_in:
        ...
        for line in lines[1:]:
            type_work = line.strip().split(",")
            response = send(
                BASE_URL + "typeworks/workStatus/update",
                {
            ...
            if response.status_code == 200:
                uploaded = uploaded + 1
        print(
            "Relações criadas: Tipos de obras - Estados das obras: {0}".format(uploaded)
        )
...
def add_comments():
    with open("notifications/comments.csv", "r") as f_in:
        ...
        for line in lines[1:]:
            ...
            response = send(
                BASE_URL + "notification/add/comments",
                {
...
def add_users():
    with open("users/users.csv", "r") as f_in:
        ...
        for line in lines[1:]:
            ...
            response = send(
                BASE_URL + "security/users/create",
                {
                    "email": user[0],
...
def add_admin_user():
    response = send(
        BASE_URL + "security/users/create/admin",
        {
