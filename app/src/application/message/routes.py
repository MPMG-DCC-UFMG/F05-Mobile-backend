...
class MessageRouter(BaseRouter):
    ...
    @classmethod
    @message_router.get("/")
    async def get_messages_from_call(
    ...
    async def mark_message_as_readed(
        message_id, db: Session = Depends(get_db)
    ) -> Message:
        return repository.mark_message_as_readed(db, message_id)
