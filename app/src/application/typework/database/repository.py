...
def get_type_workd_by_flag(flag: int, db: Session) -> TypeWork:
    return db.query(TypeWorkDB).filter(TypeWorkDB.flag == flag).first()
...
def update_type_work(db: Session, type_work: TypeWork) -> TypeWork:
    db_type_work = (
        db.query(TypeWorkDB).filter(TypeWorkDB.flag == type_work.flag).first()
    )
    if db_type_work:
...
def add_type_photo_to_type_work(
    db: Session, type_work_id: int, type_photo_id: int
) -> bool:
    db_type_work = db.query(TypeWorkDB).filter(TypeWorkDB.flag == type_work_id).first()
    db_type_photo = (
        db.query(TypePhotoDB).filter(TypePhotoDB.flag == type_photo_id).first()
    )
...
def update_type_photos_to_type_work(
    db: Session, type_work_id: int, type_photos: List[int]
) -> bool:
    db_type_work = db.query(TypeWorkDB).filter(TypeWorkDB.flag == type_work_id).first()
...
def update_work_statuses_of_type_work(
    db: Session, type_work_id: int, work_statuses: List[int]
) -> bool:
    db_type_work = db.query(TypeWorkDB).filter(TypeWorkDB.flag == type_work_id).first()
    db_work_statuses = db.query(WorkStatusDB).filter(
        WorkStatusDB.flag.in_(work_statuses)
    )
