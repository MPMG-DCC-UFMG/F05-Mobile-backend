...
from application.notifications.models.notification import Notification, PushNotification
...
class NotificationRouter(BaseRouter):
    ...
    async def add_notification(
        notification: Notification, db: Session = Depends(get_db)
    ) -> Notification:
        return repository.add_notification(db, notification)

    @staticmethod
    @notification_router.post("/push", dependencies=[Depends(admin_role)])
    async def send_push_notification(notification: PushNotification):
        ...
        if res.status_code == 200:
            ...
            raise HTTPException(status_code=404, detail="Phone token not found")

    @staticmethod
    @notification_router.get("/{notification_id}")
    async def get_notification_from(
